...
`
	mockPlainClone := func(path string, _ bool, _ *git.CloneOptions) (*git.Repository, error) {
		if err := ioutil.WriteFile(filepath.Join(path, "config.yaml"), []byte(config), 0644); err != nil {
			return nil, err
		}
		return nil, nil
	}
	...
	assert.Equal(t, "http://test.com/workflows-catalog", catalog.Workflows[0].Dependencies[0].Catalog, "got an unexpected depedency catalog")
...
func TestAddApplicationVars(t *testing.T) {
	defer patchToolchainRoot()()
	...
	data, err := ioutil.ReadFile(path.Join(app.path(), "templates", "vars-configmap.yaml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestAddApplicationSecrets(t *testing.T) {
	defer patchToolchainRoot()()
	...
	data, err := ioutil.ReadFile(path.Join(app.path(), "templates", "secrets-secret.yaml"))
	if err != nil {
		t.Fatal(err)
	}
