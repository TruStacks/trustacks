...
func (tc *toolchain) installComponents() error {
	slug := fmt.Sprintf("trustacks-toolchain-%s", tc.name)
	components, err := ioutil.ReadDir(tc.componentsPath())
	if err != nil {
		return err
	}
	...
	for _, component := range components {
		...
		go func(name string, wg *sync.WaitGroup) {
			values, err := ioutil.ReadFile(filepath.Join(tc.componentsPath(), name, "override-values.yaml"))
			if err != nil {
...
func (tc *toolchain) createAgeKeySecret() error {
	...
	return ioutil.WriteFile(path.Join(tc.path(), "chart", "templates", "sops-age-secret.yaml"), yml, 0644)
}
...
func newToolchain(name, source, version string, force bool, cloneFunc func(string, bool, *git.CloneOptions) (*git.Repository, error)) (*toolchain, error) {
	...
	if _, err := os.Stat(toolchain.path()); !os.IsNotExist(err) && !force {
		return nil, fmt.Errorf("error: toolchain '%s' already exists", name)
	}
	if force {
		if err := os.RemoveAll(toolchain.path()); err != nil {
			return nil, err
		}
	}
	...
	if err := toolchain.createAgeKeySecret(); err != nil {
		return nil, err
	}
...
type toolchainConfig struct {
	...
	Version      string                 `json:"version"`
	Parameters   map[string]interface{} `json:"parameters"`
	Applications []applicationConfig    `json:"application"`
