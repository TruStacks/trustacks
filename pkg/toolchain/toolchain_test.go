...
func TestNewToolchain(t *testing.T) {
	...
	mockPlainClone := func(basePath string, _ bool, _ *git.CloneOptions) (*git.Repository, error) {
		...
		if err := ioutil.WriteFile(path.Join(basePath, "config.yaml"), []byte(""), 0644); err != nil {
			return nil, err
		}
		return nil, nil
	}
...
password: password`,
	...
	values, err := ioutil.ReadFile(path.Join(tc.componentsPath(), "helloworld", "override-values.yaml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateAgeKeySecret(t *testing.T) {
	...
	data, err := ioutil.ReadFile(path.Join(tc.path(), "chart", "templates", "sops-age-secret.yaml"))
	if err != nil {
		t.Fatal(err)
	}
